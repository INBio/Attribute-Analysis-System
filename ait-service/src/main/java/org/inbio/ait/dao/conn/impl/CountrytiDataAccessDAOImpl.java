...
import org.inbio.ait.jdbc.mapper.CountrytiMapper;
 ...
 * @author esmata
 */
public class CountrytiDataAccessDAOImpl implements CountrytiDataAccessDAO{

    private JdbcTemplate jdbcTemplate;

    /**
     * Method to get a list of all fields from the mapped taxon indicator country table
    ...
    public List<Countryti> getAllCountriesti(CountrytiPropertyHolder ph,int limit,int offset) {
        List<Countryti> result = new ArrayList<Countryti>();
        try {
            //Setting up the jdbcTemplate
            this.accessToDB(ph);
            ...
            return this.jdbcTemplate.query(getQuery, new CountrytiMapper(ph));
        } catch (Exception e) {
            return result;
        }
    }
    ...
    private void accessToDB(CountrytiPropertyHolder ph) {
