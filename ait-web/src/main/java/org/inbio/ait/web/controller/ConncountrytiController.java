 ...
 * @author esmata
 */
public class ConncountrytiController extends SimpleFormController{
     ...
     */
    public ConncountrytiController() {
        setCommandClass(CountrytiPropertyHolder.class);
        setCommandName("connection");
    }

    /**
     * Seting the current CountrytiPropertyHolder Object to the form
     * @param request
     * @return
    ...
    protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response,
		...
		CountrytiPropertyHolder connection = (CountrytiPropertyHolder) command;

        //Persist the connection attributes
        boolean savePropertiesOk = this.configManager.saveToPropertiesFileCountryti(connection);
        //Retrive a total count from db for the specified table (test)
        int total = this.configManager.CountCountryti();
